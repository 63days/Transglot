...
from transglot.simple_utils import unique_rows
from transglot.language.constants import dialogue_symbol, end_of_sentence_symbol,\
    unknown_symbol, special_symbols
from transglot.language.helper import substitute_num_to_alpha
from transglot.language.tokenizers import replace_based_on_dic, get_naive_tokenizer_substitute_dict,\
    remove_apostrophes_from_enclosed_words
from transglot.language.parts_of_speech import transform_tokens_with_super_comparatives
...
#         docs = nlp.pipe([unicode(s, 'utf-8') for s in game_data['text']], batch_size=50)
        docs = nlp.pipe([s for s in game_data['text']], batch_size=50)
