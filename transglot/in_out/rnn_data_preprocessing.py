...
from transglot.in_out.numpy_dataset import NumpyDataset
from transglot.simple_utils import sort_and_count_len_of_dict_values
from transglot.in_out.game_data_preprocessing import condition_mask_of_data
from transglot.in_out.geometry import group_target_geometries, group_geometries,\
...
def make_dataset_for_rnn_based_model(game_data, splits, max_seq_len, only_correct=False,
    ...
    geo_ids, labels, padded_text, seq_len, mask = prepare_rnn_dataset(game_data, max_seq_len,
                                                                      ...
                                                                      geo_condition, seed, only_easy, part_anno)
...
def prepare_rnn_dataset(game_data, max_seq_len, only_correct=False, drop_too_long=True,
                        geo_condition=None, seed=None, only_easy=True, part_anno=False):
    ...
    if only_easy:
        ...
        context_mask = np.where(context=="easy", True, False)
        mask = np.logical_and(mask, context_mask)

    if part_anno:
        data_dir = "/home/ubuntu/projects/transglot/data/main_data_for_chairs"
        no_part_dataset_list = np.loadtxt(osp.join(data_dir, "no_in_part_dataset_list.txt"), dtype=np.int64)
        no_part_set = set(no_part_dataset_list)
        no_part_mask = np.ones(game_data.shape[0])
        chairs = np.array(game_data[["chair_a", "chair_b", "chair_c"]])
        for i in range(game_data.shape[0]):
            chairs_per_chat = chairs[i]
            chair_set = set(chairs_per_chat)
            if not chair_set.isdisjoint(no_part_set):
                no_part_mask[i] = 0
        mask = np.logical_and(mask, no_part_mask)
