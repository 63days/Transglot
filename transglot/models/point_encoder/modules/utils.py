...
def square_distance(src, tgt):
    ...
    Input:
        ...
        tgt: [B, M, C]
    Output:
        dist: square distance, [B, N, M]
    """
    return torch.sum((src[:,:,None] - tgt[:,None]) ** 2, dim=-1)
...
def gather_knn(xyz, idx):
    original_idx_shape = idx.shape
    stretched_idx = idx.flatten(1)
    sample = torch.gather(xyz, 1, stretched_idx[...,None].repeat(1, 1, xyz.size(2)))
    return sample.reshape(*original_idx_shape, -1)

def trilinear_interpolation(sample_feature, sample_xyz, xyz):
    '''
    Input:
        ...
        xyz: [B,N,3]
    Output:
        interpolated_feature: [B,N,D]
    '''
    weights, idx = three_nn(xyz, sample_xyz)
    sample_feature = sample_feature.transpose(1,2).contiguous()
    return three_interpolate(sample_feature, idx.contiguous(), weights.contiguous()).transpose(1, 2).contiguous()